...
async function getFetch() {
  ...
  _fetch = (mod.default || mod) as unknown as typeof fetch;
  return _fetch;
}

async function postClick(country: string) {
...
async function getTrending(limit = 10, decay?: number) {
  const fetch = await getFetch();
  const q = [`limit=${limit}`];
  if (decay !== undefined) q.push(`decay=${decay}`);
